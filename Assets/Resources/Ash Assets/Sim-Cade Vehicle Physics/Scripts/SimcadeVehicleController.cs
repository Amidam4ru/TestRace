...
namespace Ashsvp
{
    [RequireComponent(typeof(PlayerControllerReader))]
    public class SimcadeVehicleController : MonoBehaviour
    {
        ...
        private PlayerControllerReader _playerControllerReader;
        ...
        void Awake()
        {
            ...
            _playerControllerReader = GetComponent<PlayerControllerReader>();
        }
        ...
        private void Update() // Получаем вектора движения из контроллера
        {
            if (CanDrive && CanAccelerate)
            {
                accelerationInput = _playerControllerReader.MovementInput.y;
                steerInput = _playerControllerReader.MovementInput.x;
                brakeInput = _playerControllerReader.DriftInput;
            }
